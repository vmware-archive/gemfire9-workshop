    ...
    $.fn.split = function(options) {
        ...
        if (spliters.length == 0) { // first time bind events to document
            $(document.documentElement).bind('mousedown.spliter', function() {
				...
				//alert('custom-scroll');
                var h1 = $(".leftTopSplitterSpacing").height() 
                        - $(".rightInnerBlocks").height() 
                        - $(".canvasBlockInnerBlock").height() 
                        - $(".regionNameSearch").height(); 
                h1 = h1 - 10;

                $('.leftSliderScrollRegion.ScrollPaneBlock').height(h1);

                var h2 = $(".bottom_panel").height() 
                        - $(".rightInnerBlocks").height();
                h2 = h2 - 2;

                $(".leftSliderScrollMembers.ScrollPaneBlock").height(h2);

                $('.leftSliderScrollRegion.ScrollPaneBlock').jScrollPane();
                $(".leftSliderScrollMembers.ScrollPaneBlock").jScrollPane();
